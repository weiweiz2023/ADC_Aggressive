...
class gradientFilter(Function):
    ...
    def backward(ctx, grad_output):
        ...
        scale1 = torch.clamp(0.1 + torch.log(input_tens+1) / 2.5,max=1.0)
        scale2 = torch.clamp(0.1 + torch.log(-input_tens) / 2.5,max=1.0)
        scale3 = torch.clamp( -torch.sin(input_tens*torch.pi)*0.9+0.1,max=1.0)
        grad_out = torch.where(
            input_tens > 0,
            scale1*grad_output,  
            torch.where(
                (input_tens > -1) & (input_tens <= 0),
                scale3*grad_output,       # 0 < x ≤ 1
                 scale2*grad_output   # x ≤ 0
            )
