...
class quantized_conv(nn.Module):
    def __init__(self, in_channels, out_channels, arch_args, 
        ...
        w_states = 2 ** self.w_bits_per_slice - 1
        a_states = 2 ** self.a_bits_per_stream - 1
    ...
    def forward(self, inputs):
        ...
        if self.num_subarrays > 0:  # Indicate use of reg conv2d (no subarrays)
            output, a_loss = self.partial_sum_conv(qa, qw)
        else:
